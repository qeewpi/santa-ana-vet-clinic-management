      ...
      <div className="px-12">
        ...
        <div className="flex justify-center items-end w-full overflow-hidden ">
          <img
            src={catAndDogImage}
            alt="catAndDogImage"
            className="absolute flex justify-center items-center object-cover"
          />
        </div>
      </div>
      {theme.theme === "light" ? (
        <div class="absolute inset-0 -z-10 h-full w-full bg-white [background:radial-gradient(125%_125%_at_50%_10%,#fff_40%,#735C3C_100%)]"></div>
      ) : (
        <div class="absolute inset-0 -z-10 h-full w-full items-center px-5 py-24 [background:radial-gradient(125%_125%_at_50%_10%,#000_40%,#735C3C_100%)]"></div>
      )}
    </div>
  );
}
